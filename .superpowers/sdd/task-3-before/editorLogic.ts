...
const formatSceneCardDuration = (durationInFrames: number, fps: number): string => {
...
export const createSceneMediaItems = ({
  ...
  const baseLabel = getSceneCardLabelBase(label);
  return ranges.flatMap((range, index) => {
    ...
    return [{
      ...
      label: `${baseLabel} - Scene ${sceneIndex}`,
      src,
...
export const splitSceneMediaItemAtFrame = ({
  ...
  const baseLabel = getSceneCardLabelBase(target.label);
  const renumbered = new Map(sourceItems.map((item, index) => [
    item.id,
    {
      ...item,
      label: `${baseLabel} - Scene ${index + 1}`,
      sceneIndex: index + 1,
    },
  ]));
...
export const getManualRotationAngle = (
  ...
  const degrees = Math.round((pointerAngle * 180) / Math.PI + 90 + rotationOffset);
...
export const resetCutoutMask = (
  ...
  const nextClips = clips.map((clip) => {
    ...
    changed = true;
    const effectiveOriginalSourceStart = getEffectiveCutoutOriginalSourceStart(clip);
...
export const getTextAnimationPresentation = (
  ...
  if (preset === "bounce" || preset === "star-jump" || preset === "typewriter" || preset === "wave") {
    return neutralTextAnimation;
  }
...
export const setClipTransitionById = (
  ...
  const incomingClipIndex = clips.findIndex((clip) => clip.id === incomingClipId);
  ...
  const orderedIncomingIndex = orderedClips.findIndex((clip) => clip.id === incomingClipId);
  const previousClip = orderedIncomingIndex > 0 ? orderedClips[orderedIncomingIndex - 1] : undefined;

  if (!previousClip || previousClip.start + previousClip.duration !== incomingClip.start) {
    return clips;
  }
...
export const removeSilenceFromLinkedVideo = (
  ...
  const speechSegments = speechRanges.flatMap((range) => {
    const sourceTimelineStart = Math.round(
      (range.startSeconds * fps) / speed,
    );
    const sourceTimelineEnd = Math.round(
      (range.endSeconds * fps) / speed,
    );
    const duration = sourceTimelineEnd - sourceTimelineStart;
    if (duration <= 0) {
      return [];
    }
  ...
  const videoSegments = speechSegments.map<TimelineClip>((segment, index) => ({
    ...(index === 0 ? videoClip : withoutClipTransition(videoClip)),
    id: `${videoClip.id}-speech-${index}`,
    start: segment.start,
    duration: segment.duration,
    sourceStart: (videoClip.sourceStart ?? 0) + segment.sourceOffset,
    linkedClipId: linkedAudio
      ? `${linkedAudio.id}-speech-${index}`
      : videoClip.linkedClipId,
  }));
  const audioSegments = linkedAudio
    ? speechSegments.map<TimelineClip>((segment, index) => ({
        ...linkedAudio,
        id: `${linkedAudio.id}-speech-${index}`,
        start: segment.start,
        duration: segment.duration,
        sourceStart: (linkedAudio.sourceStart ?? 0) + segment.sourceOffset,
        linkedClipId: `${videoClip.id}-speech-${index}`,
      }))
    : [];
...
export const reconcileClipSourceDuration = (
  ...
  const reconciled = clips.map((clip) => {
    if (clip.src !== src || !["main", "upper", "cutout", "audio"].includes(clip.track)) {
      return clip;
    }
