...
import {cp, copyFile, mkdtemp, mkdir, readFile, rm, stat, writeFile} from "node:fs/promises";
...
const toStoredMediaName = (originalName, storedExtension) => {
  const safeName = basename(originalName || "media.bin")
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 48);
  const extension = (storedExtension ?? extname(originalName || "").toLowerCase()) || ".bin";
  return `${safeName || "media"}-${randomUUID()}${extension}`;
};

const hasBrowserOnlySource = (project) =>
  [...(project.clips ?? []), ...(project.mediaItems ?? [])].some((item) =>
    typeof item?.src === "string" && item.src.startsWith("blob:"),
  );
...
export const createTranscriptionApp = ({
  ...
  const app = express();
  const processor = backgroundRemovalProcessor ?? createBackgroundRemovalProcessor({
    runFfmpeg: (args, options) => execFileImpl(ffmpegBinaryPath, args, options),
  ...
  app.post(
    ...
    async (req, res) => {
    const request = parseBackgroundRemovalRequest(req.body, maxTrimDurationSeconds);
    const matchesMediaKind =
      request?.mediaKind === "image"
        ? req.file?.mimetype.startsWith("image/")
        : request?.mediaKind === "video" && req.file?.mimetype.startsWith("video/");
    ...
    try {
      ...
      const expectedOutput = request.mediaKind === "image"
  ...
  app.post("/api/export", async (req, res) => {
    ...
    try {
      ...
      const serveUrl = await bundleImpl({
      ...
      const binariesDirectory = await getRenderBinariesDirectory();
      ...
      return res.download(outputLocation, "video-editor-export.mp4", async () => {
        if (tempDirectory) {
          await removeDirectoryImpl(tempDirectory, cleanupOptions);
        }
      });
  ...
  app.set("captionApiPort", Number.parseInt(String(captionApiPort), 10) || 5174);
